...
function showPlayerRatingDetail(name){
  ...
  // No fabricated record. A player with no rating row reads as null here, and
  // an unrated player never has a rating or an RD rendered.
  const rec = (D.ratings||{})[k] || null;
  const rated = Ratings.isRated(rec);
  const toGo = Ratings.gamesUntilRated(rec);
  const rating = rec && rec.rating != null ? rec.rating : null;
  const rd = rec && rec.rd != null ? rec.rd : null;
  const gp = (rec && rec.gamesPlayed) || 0;
  const peak = rec && rec.peakRating != null ? rec.peakRating : rating;
  const history = rec && rec.history ? Object.values(rec.history) : [];
  history.sort((a,b)=>(b.timestamp||0)-(a.timestamp||0));
  // A history entry is a game only when it carries a boolean won. Rating
  // history also holds non-game events, specifically a verification raise where
  // an admin lifts a stale rating to match an external number. Those have no
  // won, no partner, no opponents and no score, and every count below either
  // derives losses by subtraction or labels itself in games, so they have to be
  // filtered out here rather than guarded against one site at a time.
  const games = history.filter(h => h && typeof h.won === 'boolean');
  ...
  if(games.length<2){
    ...
    const sortedAsc = games.slice().sort((a,b)=>(a.timestamp||0)-(b.timestamp||0));
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px;">
      <div>
        ...
        <div style="font-size:12px;color:var(--gray);margin-top:2px;">± ${Math.round(rd)} · ${gp} game${gp===1?'':'s'} · peak ${Math.round(peak)}</div>`
          : `<div style="font-family:'Bebas Neue';font-size:42px;line-height:1;color:var(--gray);">Unrated</div>
        <div style="font-size:12px;color:var(--gray);margin-top:2px;">${rec ? (toGo > 0 ? toGo + ' more game' + (toGo===1?'':'s') + ' to get rated' : gp + ' game' + (gp===1?'':'s')) : 'No games played yet'}</div>`}
      </div>
...
function renderRatingsHeader(){
  ...
  // Average across rated players only, so provisional records do not drag it.
  const ratedOnly = all.filter(r=>Ratings.isRated(r));
  const avg = ratedOnly.length ? Math.round(ratedOnly.reduce((s,r)=>s+r.rating,0)/ratedOnly.length) : 0;
...
function renderRatings(){
  ...
  tbody.innerHTML = all.map((r,i)=>`
    <tr onclick="showPlayerRatingDetail('${String(r.name).replace(/'/g,"\\'")}')" style="cursor:pointer;">
      ...
      <td style="font-family:'Bebas Neue';font-size:18px;color:${Ratings.isRated(r)?'var(--primary)':'var(--gray)'};">${Ratings.isRated(r)?Math.round(r.rating):'Unrated'}</td>
      <td style="font-size:12px;color:var(--gray);">${Ratings.isRated(r)?'± '+Math.round(r.rd):''}</td>
